...
let positions = [createCells(10, 10)[boxIndex]];
...
function generateColorTarget(id, x, y){
    ...
    const randomColor = getColor(colorList[randomNumber], .6);
    const colorData = {
        style: randomColor,
...
function updateBoxCount() {
    console.log(boxes)
    positions = [...Array(boxIndex)].map((_, index) => createCells(10, 10)[index]);
...
function createBox(id, x, y, colorData){
    ...
    const radiusData = [
        { radius: 1, color: getColor(color.RED, .4)},
        { radius: 5, color:  getColor(color.WHITE, .4) },
        { radius: 10, color:  getColor(color.RED, .4)},
	];
